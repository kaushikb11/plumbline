...
| **WS5 adapters** (`adapters/`, `transport/`) — OM1 (proxy config, Zenoh `cmd_vel` tap, seam classification, tool-call `Move` decisions, counterfactual swap), a generic OpenAI-agent-loop adapter (bus-less, derived action seam), **plus a G1 humanoid adapter** (cross-embodiment) and an **ActionSchema-derived behavior matcher** — all proving the frozen contract is runtime- and embodiment-agnostic | ✅ implemented & tested. The OM1 adapter is **run-verified against the real OM1 Go binary** via a software-in-the-loop episode — real cloud LLM, real Zenoh `cmd_vel`, faithful replay byte-identical, all three previously-open interface facts pinned ([docs/om1-integration.md](docs/om1-integration.md), `examples/record_om1_sil.py`); a Gazebo episode still pending for sim-grounded scenes ([docs/record-om1-gazebo.md](docs/record-om1-gazebo.md), needs Ubuntu+ROS2+Gazebo) |
