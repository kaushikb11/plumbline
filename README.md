...
| **WS5 adapters** (`adapters/`, `transport/`) — OM1 (proxy config, Zenoh `cmd_vel` tap, seam classification, tool-call `Move` decisions, counterfactual swap), a generic OpenAI-agent-loop adapter (bus-less, derived action seam), **plus a G1 humanoid adapter** (cross-embodiment) and an **ActionSchema-derived behavior matcher** — all proving the frozen contract is runtime- and embodiment-agnostic | ✅ implemented & tested. The OM1 adapter is **verified against OM1's actual source** ([docs/om1-integration.md](docs/om1-integration.md)); a real Gazebo recording still pending ([docs/record-om1-gazebo.md](docs/record-om1-gazebo.md), needs Ubuntu+ROS2+Gazebo) |
